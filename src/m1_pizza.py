...
def run_test_generate_points_on_circle():
    ...
    expected = [rg.Point(225, 100), rg.Point(150, 175), rg.Point, rg.Point(75, 100), rg.Point(150, 25)]
    circle = rg.Circle(rg.Point(150, 100), 75)
    actual = generate_points_on_circle(circle, 4)
...
def run_test_draw_points_on_circle():
    ...
    # ------------------------------------------------------------------
    title = 'This should be 80 purple circles on a black circle'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for variable in centers:
        little_circles = rg.Circle(variable, 10)
        little_circles.fill_color = color
        little_circles.attach_to(window)
        variable.attach_to(window)
    window.render()
...
def run_test_pizza():
    ...
    title = 'PIZZA test 4:  8 thin slices (thickness=4), black lines, yellow-fill.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(200, 200), 150)
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    # ------------------------------------------------------------------
    circle.attach_to(window)
    perimeter_point_list = generate_points_on_circle(circle, number_of_slices)
    for k in range(number_of_slices):
        lines = rg.Line(circle.center, perimeter_point_list[k])
        lines.color = color
...
def run_test_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'Test #3: 4 segments with thick green lines; 4 segments with medium purple lines'
    window = rg.RoseWindow(600, 600, title)

    circle = rg.Circle(rg.Point(250, 250), 200)
    circle.outline_thickness = 4
    circle.fill_color = 'lime green'
    polygon(window, circle, 4, 'magenta', 4)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    # d = 2
    # circle.attach_to(window)
    # points_list = generate_points_on_circle(circle, number_of_segments + c)
    # for k in range(number_of_segments):
    #     lines = rg.Line(points_list[k], points_list[(k + 1) % (len(points_list) - d)])
    #     lines.color = color
    #     lines.thickness = thickness
    #     lines.attach_to(window)
    # window.render()
    # You can get some wild pictures by modifying
    circle.attach_to(window)
    ...
    for k in range(number_of_segments):
        ...
        lines.attach_to(window)
    window.render()
    # This literally took you 20 minutes. To figure out what the modulus would be. Because you assumed that it took 4
    #  lines to connect *5* points, instead of x(lines) :: x(points).
...
def run_test_fancy_polygon():
    ...
    circle = rg.Circle(rg.Point(240, 165), 150)
    fancy_polygon(window, circle, 51, 25, 'black', 5)
    window.close_on_mouse_click()
